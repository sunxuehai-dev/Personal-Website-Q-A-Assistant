(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const n of a)if(n.type==="childList")for(const c of n.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&o(c)}).observe(document,{childList:!0,subtree:!0});function t(a){const n={};return a.integrity&&(n.integrity=a.integrity),a.referrerPolicy&&(n.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?n.credentials="include":a.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function o(a){if(a.ep)return;a.ep=!0;const n=t(a);fetch(a.href,n)}})();function w(e,s,t){const o=e.error??{},a=o.code??"request_failed",n=e.detail??o.message??t;return a==="chat_capacity_full"||s===429?new Error("当前聊天请求较多，容量已满，请稍后重试。"):a==="upload_busy"||s===409?new Error("系统正在处理另一份上传文档，请稍后再试。"):a==="upload_too_large"||s===413?new Error("上传文件过大，已超过当前站点限制。"):a==="unsupported_file_type"?new Error("当前仅支持上传 PDF 文件。"):a==="empty_question"?new Error("请输入问题后再发送。"):a==="model_timeout"?new Error("模型响应超时，请稍后重试，或改成更短的问题。"):new Error(n)}async function h(e,s){const t=await fetch(e,s);if(!t.ok){const o=await t.json().catch(()=>({}));throw w(o,t.status,s?.fallbackMessage??"请求失败")}return t.json()}function k(){return h("/site_content",{fallbackMessage:"无法获取站点内容"})}function E(){return h("/upload_status",{fallbackMessage:"无法获取上传状态"})}function I(){return h("/runtime_status",{fallbackMessage:"无法获取运行时状态"})}function U(){return h("/upload_status",{method:"DELETE",fallbackMessage:"无法清空上传文档"})}function A(e){return h(`/session/${encodeURIComponent(e)}`,{method:"DELETE",fallbackMessage:"无法清空会话"})}function D(e,s,t){return h("/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:e,use_uploaded_docs:s,session_id:t}),fallbackMessage:"问答请求失败"})}async function M(e,s,t){const o=await fetch("/chat_stream",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:e,use_uploaded_docs:s,session_id:t})});if(!o.ok){const a=await o.json().catch(()=>({}));throw w(a,o.status,"问答请求失败")}return o}async function T(e){const s=new FormData;return s.append("file",e),h("/upload_resume",{method:"POST",body:s,fallbackMessage:"上传失败"})}const $="resume_assistant_frontend_session_id";function j(){const e=globalThis.crypto?.randomUUID?.();if(e)return e;const s=Date.now().toString(36),t=Math.random().toString(36).slice(2,10);return`session-${s}-${t}`}const y=window.localStorage.getItem($)||j();window.localStorage.setItem($,y);const i={hasUploadedDocs:!1,activeUploadFile:null,isSending:!1,isUploading:!1,lastSessionResetAt:!1};function x(e,s=180){const t=e.replace(/\s+/g," ").trim();return t.length<=s?t:`${t.slice(0,s)}...`}function r(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function P(e){const s=document.getElementById("app");s&&(document.title=`${e.profile.name} | ${e.profile.title}`,s.innerHTML=`
  ...
  `,F())}function d(e,s,t="default"){const o=document.getElementById(e);o&&(o.textContent=s,o.className=`status-chip${t==="default"?"":` ${t}`}`)}function u(e,s){const t=document.getElementById("chat-log");if(!t)return null;const o=document.createElement("div");return o.className=`message ${e}`,o.innerHTML=`<div class="message-body">${r(s)}</div>`,t.appendChild(o),t.scrollTop=t.scrollHeight,o}function _(e,s){if(!e)return;e.querySelector(".message-meta")?.remove();const t=[s.source_badge,s.used_local_context?"本地资料":"",s.used_web_search?"联网补充":"",s.retried?"已重试":""].filter(Boolean),o=s.references.slice(0,3).map(L).join(""),a=document.createElement("div");a.className="message-meta",a.innerHTML=`
  ...
  `}async function f(){const[e,s]=await Promise.all([E(),I()]);if(i.hasUploadedDocs=e.has_uploaded_docs,i.activeUploadFile=e.active_file,i.isUploading?d("knowledge-status","当前模式：正在处理上传文档","busy"):i.hasUploadedDocs&&i.activeUploadFile?d("knowledge-status",`当前模式：上传文档问答（${i.activeUploadFile}）`):d("knowledge-status","当前模式：个人简历问答"),i.lastSessionResetAt?d("session-status","会话记忆：已清空，后续对话将重新积累","subtle"):d("session-status","会话记忆：当前浏览器会话已启用","subtle"),i.isSending){d("runtime-status","运行状态：正在生成回答","busy");return}if(s.upload.busy){d("runtime-status","运行状态：上传灌库处理中","busy");return}if(s.chat.busy){d("runtime-status",`运行状态：聊天容量已满（${s.chat.active}/${s.chat.max_concurrent}）`,"busy");return}d("runtime-status","运行状态：空闲","subtle")}async function B(e,s){const o=(await M(e,i.hasUploadedDocs,y)).body?.getReader();if(!o)throw new Error("流式响应不可用");const a=new TextDecoder("utf-8");let n="",c=null;const l=s.querySelector(".message-body");for(;;){const{value:m,done:g}=await o.read();if(g)break;n+=a.decode(m,{stream:!0});const p=n.split(`

`);n=p.pop()||"";for(const S of p){const b=S.trim();if(!b.startsWith("data:"))continue;const v=JSON.parse(b.slice(5).trim());v.type==="token"?l.textContent+=v.content||"":v.type==="replace"?l.textContent=v.content||"":v.type==="meta"&&(c=v)}}c&&_(s,c)}function F(){const e=document.getElementById("chat-form"),s=document.getElementById("chat-input"),t=document.getElementById("resume-upload"),o=document.getElementById("clear-upload"),a=document.getElementById("clear-session");e?.addEventListener("submit",async n=>{n.preventDefault();const c=s?.value.trim()??"";if(!c||i.isSending||i.isUploading)return;i.isSending=!0,i.lastSessionResetAt=!1,await f().catch(()=>{});const l=u("user",c);s&&(s.value="");const m=u("assistant","");try{l&&l.scrollIntoView({block:"end"}),m&&await B(c,m)}catch{try{const g=await D(c,i.hasUploadedDocs,y),p=m?.querySelector(".message-body");p&&(p.textContent=g.answer),_(m,g)}catch(g){const p=m?.querySelector(".message-body");p&&(p.textContent=`请求失败：${g.message}`)}}finally{i.isSending=!1,f().catch(()=>{})}}),t?.addEventListener("change",async n=>{const c=n.target.files?.[0];if(!(!c||i.isSending||i.isUploading)){i.isUploading=!0,await f().catch(()=>{});try{const l=await T(c);i.hasUploadedDocs=!0,i.activeUploadFile=l.file_name,u("assistant",`已完成上传并自动灌库：${l.file_name}`)}catch(l){u("assistant",`上传失败：${l.message}`)}finally{i.isUploading=!1,t&&(t.value=""),f().catch(()=>{})}}}),o?.addEventListener("click",async()=>{try{await U(),i.hasUploadedDocs=!1,i.activeUploadFile=null,u("assistant","已清空上传知识库，当前恢复为个人简历问答模式。"),f().catch(()=>{})}catch(n){u("assistant",`清空失败：${n.message}`)}}),a?.addEventListener("click",async()=>{try{await A(y),i.lastSessionResetAt=!0;const n=document.getElementById("chat-log");n&&(n.innerHTML=""),u("assistant","会话已清空，可以重新开始提问。"),f().catch(()=>{})}catch(n){u("assistant",`清空会话失败：${n.message}`)}}),f().catch(()=>{}),window.setInterval(()=>{f().catch(()=>{})},1e4)}k().then(P).catch(e=>{const s=document.getElementById("app");s&&(s.innerHTML=`<div class="shell"><section class="panel"><h1>前端骨架初始化失败</h1><p>${r(e.message)}</p></section></div>`)});
