(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const n of a)if(n.type==="childList")for(const c of n.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&o(c)}).observe(document,{childList:!0,subtree:!0});function t(a){const n={};return a.integrity&&(n.integrity=a.integrity),a.referrerPolicy&&(n.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?n.credentials="include":a.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function o(a){if(a.ep)return;a.ep=!0;const n=t(a);fetch(a.href,n)}})();function w(e,s,t){const o=e.error??{},a=o.code??"request_failed",n=e.detail??o.message??t;return a==="chat_capacity_full"||s===429?new Error("当前聊天请求较多，容量已满，请稍后重试。"):a==="upload_busy"||s===409?new Error("系统正在处理另一份上传文档，请稍后再试。"):a==="upload_too_large"||s===413?new Error("上传文件过大，已超过当前站点限制。"):a==="unsupported_file_type"?new Error("当前仅支持上传 PDF 文件。"):a==="empty_question"?new Error("请输入问题后再发送。"):a==="model_timeout"?new Error("模型响应超时，请稍后重试，或改成更短的问题。"):new Error(n)}async function g(e,s){const t=await fetch(e,s);if(!t.ok){const o=await t.json().catch(()=>({}));throw w(o,t.status,s?.fallbackMessage??"请求失败")}return t.json()}function E(){return g("/site_content",{fallbackMessage:"无法获取站点内容"})}function k(){return g("/upload_status",{fallbackMessage:"无法获取上传状态"})}function I(){return g("/runtime_status",{fallbackMessage:"无法获取运行时状态"})}function A(){return g("/upload_status",{method:"DELETE",fallbackMessage:"无法清空上传文档"})}function U(e){return g(`/session/${encodeURIComponent(e)}`,{method:"DELETE",fallbackMessage:"无法清空会话"})}function D(e,s,t){return g("/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:e,use_uploaded_docs:s,session_id:t}),fallbackMessage:"问答请求失败"})}async function M(e,s,t){const o=await fetch("/chat_stream",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:e,use_uploaded_docs:s,session_id:t})});if(!o.ok){const a=await o.json().catch(()=>({}));throw w(a,o.status,"问答请求失败")}return o}async function x(e){const s=new FormData;return s.append("file",e),g("/upload_resume",{method:"POST",body:s,fallbackMessage:"上传失败"})}const _="resume_assistant_frontend_session_id",y=window.localStorage.getItem(_)||crypto.randomUUID();window.localStorage.setItem(_,y);const i={hasUploadedDocs:!1,activeUploadFile:null,isSending:!1,isUploading:!1,lastSessionResetAt:!1};function P(e,s=180){const t=e.replace(/\s+/g," ").trim();return t.length<=s?t:`${t.slice(0,s)}...`}function l(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function L(e){const s=document.getElementById("app");s&&(document.title=`${e.profile.name} | AI 控制台`,s.innerHTML=`
    <div class="shell dashboard-shell">
      <section class="hero hero-console">
        <div class="hero-copy">
          ...
          <p class="subtitle">${l(e.profile.name)} · ${l(e.profile.title)}</p>
          <p class="copy">${l(e.profile.tagline)}。当前版本已经完成前后端分离第二阶段，独立前端工程通过 API 直接接入问答、上传、状态与会话能力。</p>
          <div class="hero-tags">
        ...
        <div class="hero-signal">
          ...
          <div class="signal-grid">
            ...
            <div class="signal-card">
              ...
              <strong>${l(e.profile.location)}</strong>
              <p>${l(e.profile.email)}</p>
            </div>
            <div class="signal-card">
          ...
          <div class="timeline">
            ${e.experience.map(t=>`
                  <div class="timeline-item">
                    <span class="timeline-period">${l(t.period)}</span>
                    <div class="timeline-content">
                      <strong>${l(t.role)}</strong>
                      <p>${l(t.company)}</p>
                      <small>${l(t.summary)}</small>
                    </div>
                  </div>
                `).join("")}
          </div>
      ...
      <section class="workspace-grid">
        ...
        <aside class="workspace-side">
          <section class="panel side-panel">
            ...
            <div class="chip-row">
              ${e.skills.map(t=>`<span class="chip">${l(t)}</span>`).join("")}
            </div>
          </section>
  ...
  `,R())}function d(e,s,t="default"){const o=document.getElementById(e);o&&(o.textContent=s,o.className=`status-chip${t==="default"?"":` ${t}`}`)}function u(e,s){const t=document.getElementById("chat-log");if(!t)return null;const o=document.createElement("div");return o.className=`message ${e}`,o.innerHTML=`<div class="message-body">${l(s)}</div>`,t.appendChild(o),t.scrollTop=t.scrollHeight,o}function $(e,s){if(!e)return;e.querySelector(".message-meta")?.remove();const t=[s.source_badge,s.used_local_context?"本地资料":"",s.used_web_search?"联网补充":"",s.retried?"已重试":""].filter(Boolean),o=s.references.slice(0,3).map(T).join(""),a=document.createElement("div");a.className="message-meta",a.innerHTML=`
    <div class="meta-badges">
      ${t.map(n=>`<span>${l(n)}</span>`).join("")}
    </div>
    ${o?`<div class="reference-list">${o}</div>`:""}
  `,e.appendChild(a)}function T(e){const s=[e.source_file||"未知来源",e.page?`P${e.page}`:"",e.retrieval_method||""].filter(Boolean);return`
    <div class="reference-card">
      <strong>${l(s.join(" · "))}</strong>
      <p>${l(P(e.content))}</p>
    </div>
  `}async function f(){const[e,s]=await Promise.all([k(),I()]);if(i.hasUploadedDocs=e.has_uploaded_docs,i.activeUploadFile=e.active_file,i.isUploading?d("knowledge-status","当前模式：正在处理上传文档","busy"):i.hasUploadedDocs&&i.activeUploadFile?d("knowledge-status",`当前模式：上传文档问答（${i.activeUploadFile}）`):d("knowledge-status","当前模式：个人简历问答"),i.lastSessionResetAt?d("session-status","会话记忆：已清空，后续对话将重新积累","subtle"):d("session-status","会话记忆：当前浏览器会话已启用","subtle"),i.isSending){d("runtime-status","运行状态：正在生成回答","busy");return}if(s.upload.busy){d("runtime-status","运行状态：上传灌库处理中","busy");return}if(s.chat.busy){d("runtime-status",`运行状态：聊天容量已满（${s.chat.active}/${s.chat.max_concurrent}）`,"busy");return}d("runtime-status","运行状态：空闲","subtle")}async function F(e,s){const o=(await M(e,i.hasUploadedDocs,y)).body?.getReader();if(!o)throw new Error("流式响应不可用");const a=new TextDecoder("utf-8");let n="",c=null;const r=s.querySelector(".message-body");for(;;){const{value:m,done:v}=await o.read();if(v)break;n+=a.decode(m,{stream:!0});const p=n.split(`

`);n=p.pop()||"";for(const S of p){const b=S.trim();if(!b.startsWith("data:"))continue;const h=JSON.parse(b.slice(5).trim());h.type==="token"?r.textContent+=h.content||"":h.type==="replace"?r.textContent=h.content||"":h.type==="meta"&&(c=h)}}c&&$(s,c)}function R(){const e=document.getElementById("chat-form"),s=document.getElementById("chat-input"),t=document.getElementById("resume-upload"),o=document.getElementById("clear-upload"),a=document.getElementById("clear-session");e?.addEventListener("submit",async n=>{n.preventDefault();const c=s?.value.trim()??"";if(!c||i.isSending||i.isUploading)return;i.isSending=!0,i.lastSessionResetAt=!1,await f().catch(()=>{});const r=u("user",c);s&&(s.value="");const m=u("assistant","");try{r&&r.scrollIntoView({block:"end"}),m&&await F(c,m)}catch{try{const v=await D(c,i.hasUploadedDocs,y),p=m?.querySelector(".message-body");p&&(p.textContent=v.answer),$(m,v)}catch(v){const p=m?.querySelector(".message-body");p&&(p.textContent=`请求失败：${v.message}`)}}finally{i.isSending=!1,f().catch(()=>{})}}),t?.addEventListener("change",async n=>{const c=n.target.files?.[0];if(!(!c||i.isSending||i.isUploading)){i.isUploading=!0,await f().catch(()=>{});try{const r=await x(c);i.hasUploadedDocs=!0,i.activeUploadFile=r.file_name,u("assistant",`已完成上传并自动灌库：${r.file_name}`)}catch(r){u("assistant",`上传失败：${r.message}`)}finally{i.isUploading=!1,t&&(t.value=""),f().catch(()=>{})}}}),o?.addEventListener("click",async()=>{try{await A(),i.hasUploadedDocs=!1,i.activeUploadFile=null,u("assistant","已清空上传知识库，当前恢复为个人简历问答模式。"),f().catch(()=>{})}catch(n){u("assistant",`清空失败：${n.message}`)}}),a?.addEventListener("click",async()=>{try{await U(y),i.lastSessionResetAt=!0;const n=document.getElementById("chat-log");n&&(n.innerHTML=""),u("assistant","会话已清空，可以重新开始提问。"),f().catch(()=>{})}catch(n){u("assistant",`清空会话失败：${n.message}`)}}),f().catch(()=>{}),window.setInterval(()=>{f().catch(()=>{})},1e4)}E().then(L).catch(e=>{const s=document.getElementById("app");s&&(s.innerHTML=`<div class="shell"><section class="panel"><h1>前端骨架初始化失败</h1><p>${l(e.message)}</p></section></div>`)});
