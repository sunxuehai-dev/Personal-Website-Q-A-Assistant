(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const n of a)if(n.type==="childList")for(const c of n.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&o(c)}).observe(document,{childList:!0,subtree:!0});function s(a){const n={};return a.integrity&&(n.integrity=a.integrity),a.referrerPolicy&&(n.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?n.credentials="include":a.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function o(a){if(a.ep)return;a.ep=!0;const n=s(a);fetch(a.href,n)}})();function w(e,t,s){const o=e.error??{},a=o.code??"request_failed",n=e.detail??o.message??s;return a==="chat_capacity_full"||t===429?new Error("当前聊天请求较多，容量已满，请稍后重试。"):a==="upload_busy"||t===409?new Error("系统正在处理另一份上传文档，请稍后再试。"):a==="upload_too_large"||t===413?new Error("上传文件过大，已超过当前站点限制。"):a==="unsupported_file_type"?new Error("当前仅支持上传 PDF 文件。"):a==="empty_question"?new Error("请输入问题后再发送。"):a==="model_timeout"?new Error("模型响应超时，请稍后重试，或改成更短的问题。"):new Error(n)}async function h(e,t){const s=await fetch(e,t);if(!s.ok){const o=await s.json().catch(()=>({}));throw w(o,s.status,t?.fallbackMessage??"请求失败")}return s.json()}function E(){return h("/site_content",{fallbackMessage:"无法获取站点内容"})}function U(){return h("/upload_status",{fallbackMessage:"无法获取上传状态"})}function I(){return h("/runtime_status",{fallbackMessage:"无法获取运行时状态"})}function D(){return h("/upload_status",{method:"DELETE",fallbackMessage:"无法清空上传文档"})}function A(e){return h(`/session/${encodeURIComponent(e)}`,{method:"DELETE",fallbackMessage:"无法清空会话"})}function T(e,t,s){return h("/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:e,use_uploaded_docs:t,session_id:s}),fallbackMessage:"问答请求失败"})}async function M(e,t,s){const o=await fetch("/chat_stream",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:e,use_uploaded_docs:t,session_id:s})});if(!o.ok){const a=await o.json().catch(()=>({}));throw w(a,o.status,"问答请求失败")}return o}async function j(e){const t=new FormData;return t.append("file",e),h("/upload_resume",{method:"POST",body:t,fallbackMessage:"上传失败"})}const $="resume_assistant_frontend_session_id",L="https://blog.csdn.net/sunxuehai1?spm=1000.2115.3001.5343";function x(){const e=globalThis.crypto?.randomUUID?.();if(e)return e;const t=Date.now().toString(36),s=Math.random().toString(36).slice(2,10);return`session-${t}-${s}`}const v=window.localStorage.getItem($)||x();window.localStorage.setItem($,v);const i={hasUploadedDocs:!1,activeUploadFile:null,isSending:!1,isUploading:!1,lastSessionResetAt:!1};function B(e,t=180){const s=e.replace(/\s+/g," ").trim();return s.length<=t?s:`${s.slice(0,t)}...`}function r(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#39;")}function S(){return`
  ...
  `}function P(e){const t=document.getElementById("app");t&&(document.title=`${e.profile.name} | ${e.profile.title}`,t.innerHTML=`
    <div class="shell site-shell">
      ...
      <section class="link-strip">
        <a class="panel link-card link-card-primary" href="${L}" target="_blank" rel="noreferrer">
  ...
  `}async function u(){const[e,t]=await Promise.all([U(),I()]);if(i.hasUploadedDocs=e.has_uploaded_docs,i.activeUploadFile=e.active_file,i.isUploading?d("knowledge-status","当前模式：正在处理上传文档","busy"):i.hasUploadedDocs&&i.activeUploadFile?d("knowledge-status",`当前模式：文档问答 · ${i.activeUploadFile}`):d("knowledge-status","当前模式：站内问答"),i.lastSessionResetAt?d("session-status","会话记忆：已清空","subtle"):d("session-status","会话记忆：当前浏览器会话","subtle"),i.isSending){d("runtime-status","运行状态：正在生成回答","busy");return}if(t.upload.busy){d("runtime-status","运行状态：文档处理中","busy");return}if(t.chat.busy){d("runtime-status",`运行状态：并发已满（${t.chat.active}/${t.chat.max_concurrent}）`,"busy");return}d("runtime-status","运行状态：空闲","subtle")}async function O(e,t){const o=(await M(e,i.hasUploadedDocs,v)).body?.getReader();if(!o)throw new Error("流式响应不可用");const a=new TextDecoder("utf-8");let n="",c=null;const l=t.querySelector(".message-body");for(;;){const{value:f,done:g}=await o.read();if(g)break;n+=a.decode(f,{stream:!0});const p=n.split(`

`);n=p.pop()||"";for(const k of p){const b=k.trim();if(!b.startsWith("data:"))continue;const y=JSON.parse(b.slice(5).trim());y.type==="token"?l.textContent+=y.content||"":y.type==="replace"?l.textContent=y.content||"":y.type==="meta"&&(c=y)}}c&&_(t,c)}function R(){const e=document.getElementById("chat-form"),t=document.getElementById("chat-input"),s=document.getElementById("resume-upload"),o=document.getElementById("clear-upload"),a=document.getElementById("clear-session");e?.addEventListener("submit",async n=>{n.preventDefault();const c=t?.value.trim()??"";if(!c||i.isSending||i.isUploading)return;i.isSending=!0,i.lastSessionResetAt=!1,await u().catch(()=>{});const l=m("user",c);t&&(t.value="");const f=m("assistant","");try{l&&l.scrollIntoView({block:"end"}),f&&await O(c,f)}catch{try{const g=await T(c,i.hasUploadedDocs,v),p=f?.querySelector(".message-body");p&&(p.textContent=g.answer),_(f,g)}catch(g){const p=f?.querySelector(".message-body");p&&(p.textContent=`请求失败：${g.message}`)}}finally{i.isSending=!1,u().catch(()=>{})}}),s?.addEventListener("change",async n=>{const c=n.target.files?.[0];if(!(!c||i.isSending||i.isUploading)){i.isUploading=!0,await u().catch(()=>{});try{const l=await j(c);i.hasUploadedDocs=!0,i.activeUploadFile=l.file_name,m("assistant",`文档已上传并完成入库：${l.file_name}`)}catch(l){m("assistant",`上传失败：${l.message}`)}finally{i.isUploading=!1,s&&(s.value=""),u().catch(()=>{})}}}),o?.addEventListener("click",async()=>{try{await D(),i.hasUploadedDocs=!1,i.activeUploadFile=null,m("assistant","已清空上传文档，当前恢复为站内资料问答模式。"),u().catch(()=>{})}catch(n){m("assistant",`清空失败：${n.message}`)}}),a?.addEventListener("click",async()=>{try{await A(v),i.lastSessionResetAt=!0;const n=document.getElementById("chat-log");n&&(n.innerHTML=S()),u().catch(()=>{})}catch(n){m("assistant",`清空会话失败：${n.message}`)}}),u().catch(()=>{}),window.setInterval(()=>{u().catch(()=>{})},1e4)}E().then(P).catch(e=>{const t=document.getElementById("app");t&&(t.innerHTML=`<div class="shell"><section class="panel"><h1>前端初始化失败</h1><p>${r(e.message)}</p></section></div>`)});
